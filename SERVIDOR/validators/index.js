...
module.exports = {
  validateLogin: [
    ...
    async (req, res, next) => {
      ...
      let sql = `SELECT * FROM usuarios WHERE email='${email_login}'`;

      const params = [1]; // Ejemplo de parámetro, aquí se indica que queremos usuarios activos

      let [usuario] = await executeQuery(sql, params);
     // if (!usuario) return res.error('Error al obtener usuarios', 500);
     if (!usuario){
      sql = `SELECT * FROM trabajador WHERE email='${email_login}'`;
      [usuario] =  await executeQuery(sql, params);
      if (!usuario){
        ...
        usuario.rol = 'usuarios'
     }
      if (password_login !== usuario.password) return res.error('Contraseña incorrecta', 500);
